...
```{r summaryOfSkaterByPosition, echo=FALSE}
# Function created that takes player position and outputs a numeric summary of key player stats
skateSum <- function(x=""){
  skateFun <- franSkateMod %>% filter(position==x)
  knitr::kable(cbind("avgPointsPerSeason"=round(summary(skateFun$avgPointsPerSeason),1),"avgGoalsPerSeason"=round(summary(skateFun$avgGoalsPerSeason),1),"avgAssistsPerSeason"=round(summary(skateFun$avgAssistsPerSeason),1),"avgPenaltyMinutesPerSeason"=round(summary(skateFun$avgPenaltyMinutesPerSeason),1)),caption=paste("Summary for Skater Position:",x))
}
...
```{r plots, echo=FALSE}
# Histogram creation
histSkate <- ggplot(data=franSkateMod,aes(x=avgGoalsPerSeason))
histSkate + geom_histogram()
# Boxplot creation
