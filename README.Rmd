...
franSkateMod <- franSkate %>% mutate(playerName=paste(franSkate$firstName,franSkate$lastName)) %>% arrange(desc(gamesPlayed))
franSkateMod$positionCode <- factor(franSkateMod$positionCode,c("C","D","L","R"),labels = c("Center","Defenseman","Left Wing","Right Wing"))
franSkateMod <- franSkateMod %>% rename("position"="positionCode") %>% select(playerName, playerId, activePlayer, position, seasons, gamesPlayed, points, goals, assists, penaltyMinutes, mostPointsOneSeason, mostPointsOneGame, mostGoalsOneSeason, mostGoalsOneGame, mostAssistsOneSeason, mostAssistsOneGame, mostPenaltyMinutesOneSeason)
```
